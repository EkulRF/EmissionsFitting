...
def read_spectrum(fname: str | Path) -> np.ndarray:
    """Read spectrum file
...
def create_smoother(N: int) -> np.ndarray:
...
def build_A_matrix(spectra: np.ndarray, Ns: int, Nl: int, Nt: int):
...
def read_data(
    ...
    spectral_data: str | Path,
    template_data: str | Path,
    cutoff: int = 800,
):
...
def remove_background(
    spectra: np.ndarray, bounds: dict, wv: np.ndarray, n_steps: int = 150
) -> np.ndarray:
