...
gamma = [10**(-6), 10**(-5.5), 10**(-5), 10**(-4.5), 10**(-4), 10**(-3.5), 10**(-3), 10**(-2.5), 10**(-2), 10**(-1), 10**0]
...
plt.plot(np.zeros_like(gamma), np.linspace(np.min(RMSE)-0.001, np.max(RMSE)+0.001, len(RMSE)), '-.', color='k')
plt.scatter(gamma, RMSE, color='red')
plt.xscale('log')
plt.ylabel('RMSE')
plt.xlabel('Regularisation Parameter, $\gamma$')

plt.savefig('rmse_gam.jpg')
...
for i in gamma:
    ...
    RSS.append(sum((y - y_model) ** 2))
    L2_norm.append(np.sqrt(np.sum(y_model**2)))


plt.scatter(L2_norm, RSS, c=np.log(gamma), cmap='viridis', marker='o')

cbar = plt.colorbar()
cbar.set_label('Regularisation Parameter, $\gamma$')

plt.ylabel('Residual Sum of Squares')
plt.xlabel('Euclidean Norm')

plt.savefig('L_Curve_Gamma.jpg')
