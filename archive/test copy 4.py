...
for key in Compounds:
    ...
    for i, bound in enumerate(Compounds[key]['bounds']):
        ...
        selection = np.where((W_full >= bound[0]) & (W_full <= bound[1]))

        for b in Compounds[key]['bounds']:
            indices_within_limits.append(np.max(y_model_sel[selection]))

        sigma = 0.241

        if sigma != 0:
            y = convolve(
                y_model_sel[selection], 1 * np.exp(-(W_full[selection] - np.median(W_full[selection]))**2 / (2 * sigma**2)), mode='same')

        # Plot on the left side
        axs[i][0].plot(W_full[selection], y_sel[selection], color='red', linewidth=4, label='Observed Spectra')
        axs[i][0].plot(W_full[selection], y_model_sel[selection], '-.', color='k', label='Modelled Spectra')
        axs[i][0].fill_between(W_full[selection], y_model_sel[selection] - y_model_err_sel[selection], y_model_sel[selection] + y_model_err_sel[selection], color='gray', alpha=0.5)
        ...
        diff = y_sel[selection] - y_model_sel[selection]
